...
int emCR_B()
{
    ...
    for (const auto &directory : directories)
    {
        ...
        //////// these are for emCR_B with scaled WZ ////////////
        ...
        Double_t MC = WZ + AllTop + WW + llklp + Zjets + SignalZZ;
        Double_t MC_error = sqrt(pow(WZ_error, 2) + pow(AllTop_error, 2) + pow(WW_error, 2) + pow(llklp_error, 2) + pow(Zjets_error, 2) + pow(SignalZZ_error, 2));
        ...
        Double_t dataMCerror_emCR_B = (Data / MC) * sqrt(pow(Data_error / Data, 2) + pow(MC_error / MC, 2));
        Double_t NR = AllTop;
        ...
        if (directory == "/emCR_B/")
        {
            printf("|                              |all                    |\n"
                   ...
                   Data, Data_error, EWKZZ, llvvjj_error, QCDZZ, llvv_error, WZemCR_B, WZemCR_B_error, Zjets, Zjets_error, (Zjetsee_0 + Zjetsmumu_0), sqrt(pow(Zjetsee_error_0, 2) + pow(Zjetsmumu_error_0, 2)), (Zjetsee_1 + Zjetsmumu_1), sqrt(pow(Zjetsee_error_1, 2) + pow(Zjetsmumu_error_1, 2)), (Zjetsee_2 + Zjetsmumu_2), sqrt(pow(Zjetsee_error_2, 2) + pow(Zjetsmumu_error_2, 2)), AllTop, AllTop_error, WW, WW_error, llklp, llklp_error, SignalZZ, SignalZZ_error, BKG_emCR_B, BKG_emCR_B_error, (Data / MC), dataMCerror_emCR_B, ((Data - nonNR) / NR), DatanonNRoverNR, 100 * (SignalZZ / NR), SignalZZNRerror);
            //////// Scale Factor for AllTop ///////////////////
            scaleAllTop = Numerator1 / NR;
            scaleAllTop_error = DatanonNRoverNR;
        }
        printf("The scale factor for Top is %f +- %f\n", scaleAllTop, scaleAllTop_error);
    }
    ...
    // scaling for Top
    TH1F *HTopsscaled = (TH1F *)HTops->Clone("HTopsscaled");
    ...
    TLatex *tex2 = new TLatex(0.1, 0.70, "emCR-B");
    tex2->SetNDC();
    tex2->SetTextFont(12);
    tex->SetTextSize(0.03); // Adjust the value to make the text smaller
    tex2->SetLineWidth(2);
    ...
    //leg_entry = leg->AddEntry(HTopsscaled, "top scaled", "f");
