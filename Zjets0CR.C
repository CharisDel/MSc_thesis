...
// Change of directory//
std::vector<std::string> directories = {"/Zjets0/" /*, "/emCR_B/", "/emCR_A/" , "/Zjets/" , "/SR/"*/};
...
int Zjets0CR()
{
    ...
    // scaling for Zjets0
    HZjetsscaled->Scale(scaleZjets0);

    // HTops->Scale(1.011241);
    // HZjets->Scale(1.364025);
    // HWZ->Scale(1.023562);
    // HWW->Scale(1.460575);
    ...
    HZjets->Add(HOther);
    // HZjetsscaled->Add(HOther);
    HTops->Add(HZjets);
    // HTops->Add(HZjetsscaled);
    HWW->Add(HTops);
    HWZ->Add(HWW);
    HSignal->Add(HWZ);
    ...
    TLatex *tex2 = new TLatex(0.1, 0.70, "Zjets(= 0)CR");
    tex2->SetNDC();
    tex2->SetTextFont(12);
    tex->SetTextSize(0.03); // Adjust the value to make the text smaller
    tex2->SetLineWidth(2);
    ...
    leg_entry = leg->AddEntry(HZjets, "Zjets0 scaled", "f");
    ...
    // leg_entry = leg->AddEntry(HZjetsscaled, "Zjets0 scaled", "f");
