...
int ThreelCR()
{
    ...
    // TH1F *HWZscaled = new TH1F("HWZCscaled", "", 10, 2.20,3.20);
    ...
    for (const auto &directory : directories)
    {
        ...
        Double_t QCDZZ = llvv;
        Double_t Zjets = Zjetsmumu_0 + Zjetsee_0 + Zjetsmumu_1 + Zjetsee_1 + Zjetsmumu_2 + Zjetsee_2;
        Double_t Zjets_error = sqrt(pow(Zjetsee_error_0, 2) + pow(Zjetsmumu_error_0, 2) + pow(Zjetsee_error_1, 2) + pow(Zjetsmumu_error_1, 2) + pow(Zjetsee_error_2, 2) + pow(Zjetsmumu_error_2, 2));
        ...
        //////// these are for 3lCR only ///////////////
        Double_t Alltop_WZ = scaleAlltop * AllTop;
        Double_t Alltop_WZ_error = abs(Alltop_WZ) * sqrt(pow(scaleAlltop_error / scaleAlltop, 2) + pow(AllTop_error / AllTop, 2));
        Double_t BKG = WZ + Alltop_WZ + WW + llklp + Zjets;
        Double_t BKG_error = sqrt(pow(WZ_error, 2) + pow(Alltop_WZ_error, 2) + pow(WW_error, 2) + pow(llklp_error, 2) + pow(Zjets_error, 2));
        Double_t MC = WZ + Alltop_WZ + WW + llklp + Zjets + SignalZZ;
        Double_t MC_error = sqrt(pow(WZ_error, 2) + pow(Alltop_WZ_error, 2) + pow(WW_error, 2) + pow(llklp_error, 2) + pow(Zjets_error, 2) + pow(SignalZZ_error, 2));
        ...
        Double_t dataMCerror = (Data / MC) * sqrt(pow(Data_error / Data, 2) + pow(MC_error / MC, 2));
        Double_t nonWZ = SignalZZ + llklp + WW + Alltop_WZ + Zjets;
        Double_t nonWZ_error = sqrt(pow(SignalZZ_error, 2) + pow(llklp_error, 2) + pow(WW_error, 2) + pow(Alltop_WZ_error, 2) + pow(Zjets_error, 2));
        ...
        if (directory == "/3lCR/")
        {
            printf("|                              |all                    |\n"
                   ...
                   Data, Data_error, EWKZZ, llvvjj_error, QCDZZ, llvv_error, WZ, WZ_error, Zjets, Zjets_error, (Zjetsee_0 + Zjetsmumu_0), sqrt(pow(Zjetsee_error_0, 2) + pow(Zjetsmumu_error_0, 2)), (Zjetsee_1 + Zjetsmumu_1), sqrt(pow(Zjetsee_error_1, 2) + pow(Zjetsmumu_error_1, 2)), (Zjetsee_2 + Zjetsmumu_2), sqrt(pow(Zjetsee_error_2, 2) + pow(Zjetsmumu_error_2, 2)), AllTop, AllTop_error, WW, WW_error, llklp, llklp_error, SignalZZ, SignalZZ_error, BKG, BKG_error, (Data / MC), dataMCerror, ((Data - nonWZ) / WZ), DatanonWZoverWZ, 100 * (SignalZZ / WZ), SignalZZWZerror);

            //////// Scale Factor for WZ ///////////////////
            scaleWZ = Numerator / WZ;
            scaleWZ_error = DatanonWZoverWZ;
        }
        printf("The scale factor for WZ is %f +- %f\n", scaleWZ, scaleWZ_error);
    }
    ...
    // scaling for WZ
    TH1F *HWZscaled = (TH1F *)HWZ->Clone("HWZscaled");
    ...
    //HWZscaled->Add(HWW);
    HSignal->Add(HWZ);
    //HSignal->Add(HWZscaled);

    HSignal->Draw("hist");
    HWZ->Draw("hist same");
    //HWZscaled->Draw("hist same");
    HWW->Draw("hist same");
    ...
    TLatex *tex2 = new TLatex(0.1, 0.70, "3l-CR");
    tex2->SetNDC();
    tex2->SetTextFont(12);
    tex->SetTextSize(0.03); // Adjust the value to make the text smaller
    tex2->SetLineWidth(2);
    ...
    //leg_entry = leg->AddEntry(HWZscaled, "WZ scaled", "f");
