...
int emCR_A()
{
    ...
    Double_t scaleZjets1_error = 0.037128;
    ...
    for (const auto &directory : directories)
    {
        ...
        Double_t MC = WZemCR_A + AllTop_A + WW + scaledZ0 + scaledZ1 + scaledZ2 + llklp + SignalZZ;
        Double_t MC_error = sqrt(pow(WZemCR_A_error, 2) + pow(SignalZZ_error, 2) + pow(AllTop_A_error, 2) + pow(WW_error, 2) + pow(llklp_error, 2) + pow(scaledZ0_error, 2) + pow(scaledZ1_error, 2) + pow(scaledZ2_error, 2));
        Double_t dataMCerror_emCR_A = (Data / MC) * sqrt(pow(Data_error / Data, 2) + pow(MC_error / MC, 2));
        ...
        if (directory == "/emCR_A/")
        {
            printf("|                              |all                    |\n"
                   ...
                   Data, Data_error, EWKZZ, llvvjj_error, QCDZZ, llvv_error, WZ, WZ_error, ZjetsemCR_A, ZjetsemCR_A_error, Z0, Z0_error, Z1, Z1_error, Z2, Z2_error, AllTop, AllTop_error, WW, WW_error, llklp, llklp_error, SignalZZ, SignalZZ_error, BKG_emCR_A, BKG_emCR_A_error, (Data / MC), dataMCerror_emCR_A, ((Data - nonNR_A) / NR_A), DatanonNRoverNR_A, 100 * (SignalZZ / NR_A), SignalZZNRerror_A);
            //////// Scale Factor for WW ///////////////////
            scaleWW = Numerator3 / NR_A;
            scaleWW_error = DatanonNRoverNR_A;
            cout << " The purity of emCR_A is (%): " << (100 * WW) / (WW + SignalZZ + WZ + AllTop + Z0 + Z1 + Z2 + llklp) << endl;
        }
        printf("The scale factor for WW is %f +- %f\n", scaleWW, scaleWW_error);
    }
    ...
    TLatex *tex2 = new TLatex(0.1, 0.70, "emCR-A");
    tex2->SetNDC();
    tex2->SetTextFont(12);
    tex->SetTextSize(0.03); // Adjust the value to make the text smaller
    tex2->SetLineWidth(2);
    ...
    leg_entry = leg->AddEntry(HWWscaled, "WW scaled", "f");
